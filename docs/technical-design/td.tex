...
\begin{document}
	...
	\begin{hoofdstuk}{System Overview}
		...
		\begin{paragraaf}{Implementation Strategy}
			...
			A more detailed illustration of this process can be seen in \verwijzingn{figuur}{Communication between Host and Accelerator in Co-processing Methodology}
	...
	\begin{hoofdstuk}{Integrated Block Design}
		...
		As I talked about in~\verwijzingn{paragraaf}{Memory Layout}, the BRAM usage is high, but not as high as I expected.
		...
		\begin{figuur}{FPGA Place And Route Floorplan}
			...
			\definecolor{floorplanclbblue}{HTML}{0095ff}
			\definecolor{floorplanclbyellow}{HTML}{a0c400}
	...
	\begin{hoofdstuk}{Hardware Layer}
		...
		\begin{paragraaf}{Memory Layout}
			...
			The Arty A7-35T, our initial target FPGA board, only had \qty{1800}{\kibi\bit} of BRAM, which was not enough to store a full image frame of the lowest color configuration, as can be seen in \verwijzingb{tabel}{Required space to store images of different sizes}.
			...
			Such a row of pixels is frequently called a "line buffer" or a "sliding window."
			By using this technique of only loading the pixels that we need to do a regional computation, we do not have to load the entire image into memory and thus we can make way with a smaller amount of memory.
			Unfortunately this cannot be applied to computations such as the Hough Transform because this operation needs to have computed all the votes in the accumulator of an image before it can proceed.
