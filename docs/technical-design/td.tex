...
\begin{document}
	...
	\begin{hoofdstuk}{Integrated Block Design}
		...
		It only consumed about 20 percent of the available LUTs and 30 percent of the available FFs while keeping the WNS at \qty{1,2e-9}{\second}, which is 12 percent of the clock period. % 1,2 ns :shrug:
		...
		\begin{tabel}{Post-Implementation Utilization Report}{l @{\extracolsep{\fill}} rr rr S[table-format=3.2]S[table-format=3.2]}
					& \multicolumn{2}{c}{Utilization}	& \multicolumn{2}{c}{Available} & \multicolumn{2}{c}{Utilization \%} \tabularnewline
			\cmidrule{2-3} \cmidrule{4-5} \cmidrule{6-7}
			Resource	& \multicolumn{1}{c}{VPU} & \multicolumn{1}{c}{Total} & \multicolumn{1}{c}{VPU} & \multicolumn{1}{c}{Total}		    & VPU & Total \tabularnewline
			\midrule
	...
	\begin{hoofdstuk}{Hardware Layer}
		...
		\begin{paragraaf}{Memory Layout}
			...
			The Arty A7-35T, our target FPGA board, only has \qty{1800}{\kibi\bit} of BRAM, which is not enough to store a full image frame of the lowest color configuration, as can be seen in \verwijzingb{tabel}{Required space to store images of different sizes}.
			...
			The alternatives places to store an image are the \qty{16}{\mebi\byte} of Quad-SPI flash or the \qty{256}{\mega\byte} DDR3L SDRAM.
