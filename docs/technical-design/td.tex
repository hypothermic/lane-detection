...
\begin{document}
	...
	\begin{hoofdstuk}{Benchmarks}
		...
		\begin{paragraaf}{Execution Speed}
			...
			When measuring the run time of the Python driver code, it took \qty{26}{\milli\second} ± \qty{443}{\micro\second} (mean std. dev. over 700 iterations) for the complete handling and processing of an image.
			...
			\begin{tabel}{Performance comparison per platform}{l @{\extracolsep{\fill}} S[table-format=3.1]S[table-format=3.1]}
				...
				FPGA (coprocessing)	& 26.6		& 0.443		\tabularnewline
				Reference OpenCV (CPU)	& 72.2		& 1.801		\tabularnewline
				Reference OpenCV (GPU)	& 14.2		& 0.794		\tabularnewline
				Reference Lane Program	& 382.7		& 24.16		\tabularnewline

			\end{tabel}

			\begin{figuur}{Performance comparison per platform in a bar chart}
				\begin{tikzpicture}
					\begin{axis}[
						width=\textwidth,
						height=48ex,
						...
						ymin=0,
						bar width=2.5ex,
						ylabel={milliseconds per execution},
						...
						\addplot+[
							...
							(OpenCV (GPU),		72.2) +- (0, 1.801)
							(OpenCV (CPU),		14.2) +- (0, 0.794)
							(Lane Program,		382.7) +- (0, 24.16)
						};
					\end{axis}
				\end{tikzpicture}
			\end{figuur}
